...
## Overview
The `provider` object in Cove defines the external or internal service responsible for handling each verification step. Some steps may also include a `fallback` to ensure redundancy if the primary provider fails.
